...
public partial class WasiInstance
{
    public sg_wasm.IWasmMemory Memory { get; set; }
    ...
    class FilePair
    {
     ...
     Dictionary<int,FilePair> _files = new Dictionary<int,FilePair>();

     System.IO.Stream _stdin;
     System.IO.Stream _stdout;
     System.IO.Stream _stderr;
     Stream get_stream_for_fd(int fd)
    {
        switch (fd)
        {
            case 0:
		...
		)
    {
        ...
        var path = util.from_utf8(Memory.Address + addr_path, path_len);
        System.Console.WriteLine("Open path: {0}", path);
        ...
        try
        {
            ...
            var fd = get_new_fd();
            _files[fd] = pair;
            Marshal.WriteInt32(Memory.Address + addr_fd, fd);
    ...
    public  int fd_prestat_dir_name(int fd, int addr_path, int len)
    {
        switch (fd)
        {
            case 3:
                Marshal.WriteByte(Memory.Address + addr_path, 46); // 46 is ascii for .
                //Marshal.WriteByte(sg_wasm.Address + addr_path + 1, 0); // assuming no need for z terminator
                return __WASI_ESUCCESS;
            default:
                throw new NotImplementedException(string.Format("fd {0}  len {1}", fd, len));
        }
    }
    public  int fd_prestat_get(int fd, int addr)
    {
        ...
        switch (fd)
        {
            case 3:
                ...
                Marshal.StructureToPtr(st, Memory.Address + addr, false);
                return __WASI_ESUCCESS;
            default:
                return __WASI_EBADF;
        }
     ...
     byte[][] __environ;
    public  void set_environ(Dictionary<string,string> d)
    {
    ...
    public  int environ_sizes_get(int addr_environ_count, int addr_environ_buf_size)
    {
        if (__environ != null)
        {
            Marshal.WriteInt32(Memory.Address + addr_environ_count, __environ.Length);
            int len = 0;
            foreach (var ba in __environ)
            {
                len += ba.Length;
            }
            Marshal.WriteInt32(Memory.Address + addr_environ_buf_size, len);
        }
        else
        {
            Marshal.WriteInt32(Memory.Address + addr_environ_count, 0);
            Marshal.WriteInt32(Memory.Address + addr_environ_buf_size, 0);
        }
        return __WASI_ESUCCESS;
    }
    public  int environ_get(int addr_argv, int addr_argv_buf)
    {
        int sofar = 0;
        for (int i=0; i<__environ.Length; i++)
        {
            Marshal.WriteInt32(Memory.Address + addr_argv + i * 4, addr_argv_buf + sofar);
            var ba = __environ[i];
            Marshal.Copy(ba, 0, Memory.Address + addr_argv_buf + sofar, ba.Length);
            sofar += ba.Length;
        }
        return __WASI_ESUCCESS;
    }
    ...
    public  int args_sizes_get(int addr_argc, int addr_argv_buf_size)
    {
        if (__args != null)
        {
            Marshal.WriteInt32(Memory.Address + addr_argc, __args.Length);
            int len = 0;
            foreach (var ba in __args)
            {
                len += ba.Length;
            }
            Marshal.WriteInt32(Memory.Address + addr_argv_buf_size, len);
        }
        else
        {
            Marshal.WriteInt32(Memory.Address + addr_argc, 0);
            Marshal.WriteInt32(Memory.Address + addr_argv_buf_size, 0);
        }
        return __WASI_ESUCCESS;
    }
    public  int args_get(int addr_argv, int addr_argv_buf)
    {
        int sofar = 0;
    ...
    public  void proc_exit(int a)
    {
        throw new ProcExitException(a);
    }
    public  int fd_filestat_get(int fd, int addr_result)
    {
    ...
    public  int clock_time_get(int clock_id, long precision, int addr_result)
    {
            ...
            case __WASI_CLOCK_REALTIME:
                {
                    ...
                    var ns = ms * 1000 * 1000;
                    Marshal.WriteInt64(Memory.Address + addr_result, ns);
                    return __WASI_ESUCCESS;
                }
    ...
    public  int fd_sync(int fd)
    {
        //System.Console.WriteLine("fd_sync: fd {0}", fd);
        // TODO
        return __WASI_ESUCCESS;
    }
    public  int fd_seek(int fd, long offset, int whence, int addr_newoffset)
    {
    ...
    public  int fd_read(int fd, int addr_iovecs, int iovecs_len, int addr_nread)
    {
        ...
        unsafe
        {
            a_iovecs = new Span<__wasi_iovec_t>((Memory.Address + addr_iovecs).ToPointer(), iovecs_len);
        }
        ...
        for (int i=0; i<iovecs_len; i++)
        {
            ...
            unsafe
            {
                dest = new Span<byte>((Memory.Address + (int) addr).ToPointer(), (int) len);
            }
        ...
        Marshal.WriteInt32(Memory.Address + addr_nread, total_len);

        return __WASI_ESUCCESS;
    }
    public  int poll_oneoff(int a, int b, int c, int d)
    {
        throw new NotImplementedException();
    }
    ...
    public  int fd_write(int fd, int addr_iovecs, int iovecs_len, int addr_nwritten)
    {
        //System.Console.WriteLine("fd_write: {0} {1} {2} {3}", fd, addr_iovecs, iovecs_len, addr_nwritten);
        Span<__wasi_ciovec_t> a_iovecs;
        unsafe
        {
            a_iovecs = new Span<__wasi_ciovec_t>((Memory.Address + addr_iovecs).ToPointer(), iovecs_len);
        }
        ...
        for (int i=0; i<iovecs_len; i++)
        {
            ...
            unsafe
            {
                src = new Span<byte>((Memory.Address + (int) addr).ToPointer(), (int) len);
            }
            strm.Write(src);
            total_len += (int) len;
        }

        Marshal.WriteInt32(Memory.Address + addr_nwritten, total_len);

        //System.Console.WriteLine("  done fd_write");

        return __WASI_ESUCCESS;
    }
     void write_u64(int addr, ulong v)
    {
        var ba = BitConverter.GetBytes(v);
        Marshal.Copy(ba, 0, Memory.Address + addr, ba.Length);
    }
    public  int fd_fdstat_get(int fd, int addr)
    {
    ...
    public  int fd_fdstat_set_flags(int fd, int flags)
    {
        throw new NotImplementedException();
    }
     void write_filestat(int addr, FileInfo fi)
    {
    ...
    {
        var old_path = util.from_utf8(Memory.Address + addr_old_path, old_path_len);
        var new_path = util.from_utf8(Memory.Address + addr_new_path, new_path_len);
        throw new NotImplementedException();
    }
    public  int path_unlink_file(
        int dirfd, 
        int addr_path, 
        int path_len
        )
    ...
    public  int path_remove_directory(
        int fd, 
        int addr_path, 
        int path_len
        )
    ...
    public  int path_link(int a, int b, int c, int d, int e, int f, int g)
    {
        throw new NotImplementedException();
    }
    public  int path_create_directory(
        int fd, 
        int addr_path, 
        int path_len
        )
        ...
        )
    {
        ...
        Marshal.Copy(ba, 0, Memory.Address + addr_buf, ba.Length);
        return __WASI_ESUCCESS;
    }
}